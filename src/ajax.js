...
function fetchAllHours() {
	ajax('shifts/getAll.php', {}, viewAllHours);
}	

/* Toggle Shift
Note: This function should not be called on it's own,
	  it's parameters should be automatically generated by the webpage
Parameters:
	desc - String, Description of shift to toggle
	hours - Double, Number of hours
...
function fetchIndividual(name, dialogID) {
	...
	ajax("shifts/getCompleted.php", {Name: name}, getCompleted, {Name: name, ID: dialogID});
}
...
/* View All Hours (Invoked by fetchAllHours)
Parameters:
	response - JSON, API response from backend
Global Variables:
	ajaxRequests - int, amount of AJAX requests still in progress
...
function viewAllHours(response) {
	try {
		var r = JSON.parse(response).data;
		var innerHTML = "<table class=\"displayAllTable\" id=\"exporttable\"><tr><td><b>Name</b></td><td><b>Hours</b></td></tr>";
		for (var name in r) {
			innerHTML += "<tr><td>" + name + "</td><td>" + r[name]+"</td></tr>";
		}
		innerHTML += "</table>";
		document.getElementById("viewallbody").innerHTML = innerHTML;
	} catch(e) {
		// Error Occurred
		alert(e);
	}
